    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-fade-in">
      <div className="bg-white border border-zinc-200 rounded-2xl max-w-xl w-full p-6 shadow-xl relative max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-mono text-zinc-700 mb-1.5 font-medium">
              Suspect URL <span className="text-purple-600">*</span>
            </label>
            <input
              type="url"
              required
              ...
              placeholder="https://marketplace.example/listing/12345"
              className="w-full bg-zinc-50 border border-zinc-300 rounded-lg px-3.5 py-2.5 text-xs font-mono text-zinc-950 outline-none focus:border-purple-600 focus:bg-white"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-mono text-zinc-700 mb-1.5 font-medium">
              Description of Infringing Elements (Optional)
            </label>
            <textarea
              rows={3}
              value={claimText}
          ...
          <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-200 text-[11px] text-zinc-600 leading-relaxed">
            If validators agree that this URL matches <strong className="text-purple-700">{currentStyle?.style_name}</strong> above the {currentStyle?.similarity_threshold}% threshold with commercial usage, the bounty is credited to your address automatically.
          </div>
          ...
          <div className="pt-2 flex items-center justify-end gap-3">
            ...
            <button
              ...
              className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-semibold text-xs px-5 py-2.5 rounded-lg transition-all"
            >
